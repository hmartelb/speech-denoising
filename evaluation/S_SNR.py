...
class SSNR():
    def __init__(self, estimated_source):
        #preprocess
        self.estimated_source = estimated_source
    ...
    def wada_snr(self):
        ...
        wav = np.array(self.estimated_source)
        wav = wav / abs(wav).max()
        abs_wav = abs(wav)
...
if __name__ == '__main__':
    ...
    for estimate_src in estimated_src_list:
        res = SSNR(estimate_src).wada_snr()
        ssnr_list.append(res)

    print(np.mean(ssnr_list))
