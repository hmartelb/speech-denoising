...
denoising-wavenet-pytorch (https://github.com/Sytronik/denoising-wavenet-pytorch)
...
class DWaveNet(nn.Module):
    def __init__(
        ...
        self.first_conv = nn.Conv1d(
            in_channels, residual_channels, 3, padding=1, bias=bias)
        ...
        self.last_conv_layers = nn.Sequential(
            nn.ReLU(True),
            nn.Conv1d(skip_out_channels,
                      last_channels[0], 3, padding=1, bias=bias),
            nn.ReLU(True),
            nn.Conv1d(last_channels[0], last_channels[1],
...
class ResidualConv1dGLU(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # local conditioning
        if cin_channels > 0:
            self.conv1x1c = nn.Conv1d(
                cin_channels, gate_channels, 1, bias=bias)
        else:
            self.conv1x1c = None

        # global conditioning
        if gin_channels > 0:
            self.conv1x1g = nn.Conv1d(
                gin_channels, gate_channels, 1, bias=bias)
        ...
        self.conv1x1_out = nn.Conv1d(
            gate_out_channels, residual_channels, 1, bias=bias)
        self.conv1x1_skip = nn.Conv1d(
            gate_out_channels, skip_out_channels, 1, bias=bias)
